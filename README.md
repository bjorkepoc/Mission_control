...
## Classic JARVIS skin (local fork)

This fork keeps the existing classic JARVIS-style static interface in [index.html](./index.html) with styles in [style.css](./style.css), while the full upstream backend/frontend feature set is now included in `backend/` and `frontend/`.

- Classic static UI: open `index.html`
- Full upstream app: run the stack from this README (`frontend/` + `backend/`)
